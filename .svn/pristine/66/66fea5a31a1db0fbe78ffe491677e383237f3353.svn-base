            ...
            <el-card class="box-card" shadow="never">
                <div slot="header" class="clearfix">
                    ...
                    <div class="content-list">
                        <page-table ref="table" remote="requestSchedulingList" :height="'700'" :formatPayload="formatPayload" :hidePagination="true" :row-class-name="tableRowClassName">
                        <!-- <page-table ref="table" :formatPayload="formatPayload" :row-class-name="tableRowClassName"> -->
                            ...
                            <!-- <el-table-column v-for="({name,id},index) in persons" :prop="name" :label="name" :key="index" :fixed="tableFixed(name,id)"> -->
                                <template slot-scope="scope" >
                                    <div class="col-box" :class="editCol? 'col-box-bg':''" @click="tableColumClick(scope.row[id],scope)"> 
                                    <div v-if="name !== '首席'">
                                        <el-select v-if="showSelectId == scope.row[id] && editCol && colName == scope.column.label && colDuty == scope.row.dutyDate" v-model="scope.row[id]" @change="changeTelescopic(index,scope.row[id],scope.row,name,id)" placeholder="请选择" :class="`scheduling-${scope.row[id]}`">
                                        ...
                                        <span @click="itemClick(scope.row[id])" type="text" v-else>
                                            <span :class="editCol? 'color-blue':'color-black'" v-for="item in postList" :key="item.id" v-if="scope.row[id] == item.id">{{item.name}}</span>
                                        </span>
                                    </div>
                                    ...
                                    <div v-if="name == '首席'">
                                        ...
                                        <span @click="itemClick(scope.row[id])" type="text" v-else>
                                            <span :class="editCol? 'color-blue':'color-black'" v-for="item in postUserList" :key="item.id" v-if="scope.row[id] == item.id">{{item.name}}</span>
                                        </span>
                                    </div>
                                    </div>
