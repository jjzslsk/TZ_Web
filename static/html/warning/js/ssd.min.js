...
ol.control.BaselayerSwitcher = function(a) {
	...
	ol.control.BaselayerSwitcher.prototype.baseLayers = function(a, b) {
		switch(a) {
			...
			case "offline":
				var road;
				if(this.layerUrls_.road instanceof Array) {
					road = new Array();
					for(var i = 0; i < this.layerUrls_.road.length; i++) {
						var c = layerUtil.createTiledLayer("offline", this.layerUrls_.road[i], {
							layerId: "baseLayer",
							layerCaption: "底图",
							zIndex: -1
						})
					...
					road = layerUtil.createTiledLayer("offline", this.layerUrls_.road[0], {
						layerId: "baseLayer",
						layerCaption: "底图",
						zIndex: -1
					})
				...
				var state;
				if(this.layerUrls_.sate instanceof Array) {
					var state = new Array();
					for(var i = 0; i < this.layerUrls_.sate.length; i++) {
						var d = layerUtil.createTiledLayer("offline", this.layerUrls_.sate[i], {
							layerId: "baseLayer",
							layerCaption: "底图",
							zIndex: -1
						})
					...
					state = layerUtil.createTiledLayer("offline", this.layerUrls_.sate, {
						layerId: "baseLayer",
						layerCaption: "底图",
						zIndex: -1
					})
				...
				var terr;
				if(this.layerUrls_.terr instanceof Array){
					terr = new Array();
					for(var i=0;i<this.layerUrls_.terr.length;i++){
						var e = layerUtil.createTiledLayer("offline", this.layerUrls_.terr[i], {
							layerId: "baseLayer",
							layerCaption: "底图",
							zIndex: -1
						});
					...
					terr = layerUtil.createTiledLayer("offline", this.layerUrls_.terr, {
							layerId: "baseLayer",
							layerCaption: "底图",
							zIndex: -1
						});
		...
		if(this.getVisible()) {
			...
			if("vertical" == h || "v" == h) {
				...
				"left" == f ? (m = {
					...
					b.fillRect(m.left - 70, m.top - 20, l + 30, k + 10),
					b.closePath(),
					b.beginPath(),
					...
					e[0].color ? $.each(e, function(a, c) {
						...
						b.beginPath(),
							...
							b.fillRect(d - 50, e + 20, 30, 20),
							b.rect(d - 50, e + 20, 30, 20),
							b.lineWidth = .25,
							b.strokeStyle = "#000",
							b.stroke(),
							b.fillStyle = "#000",
							b.font = g,
							b.fillText(f, d + 20, e + 30),
							b.closePath()
	...
	ol.control.CanvasTitle = function(a) {
		a || (a = {}),
			this.params_ = {
				font: a.font ? a.font : "bold 14px 宋体",
				color: a.color ? a.color : "blue",
			...
			if("TC" == this.params_.position) {
				...
				if("string" == typeof g)
					...
					for(var j = 0; j < g.length; j++)
						...
						b.textBaseline = "middle",
						b.font = h,
						b.fillText(g[j], f.left, f.top + 20 * j),
						b.closePath(),
						b.restore()
			} else {
...
/*!create by scy 2018-03-26*/
var commonUtil = {};
commonUtil.pointToGeoJson = function(a, b, c, m) {
		for(var d = {
			...
			if(!isNaN(f) && !isNaN(g) && g <= 90 && null != f & void 0 != f & null != g & void 0 != g) {
				var prop = a[e];
				if(m) { //浙江修改 用来标识feature
					for(var key in m){
						prop[key]= m[key]
					}
				}
...
contentUtil.googleUrl = {
	...
	contentUtil.tianditu = {
		road: "http://t3.tianditu.com/DataServer?T=vec_w&x={x}&y={y}&l={z}",
		sate: "http://t3.tianditu.com/DataServer?T=img_w&x={x}&y={y}&l={z}",
		terr: "http://t3.tianditu.com/DataServer?T=ter_w&x={x}&y={y}&l={z}",
		roadlabel: "http://t3.tianditu.com/DataServer?T=cva_w&x={x}&y={y}&l={z}",
		satelabel: "http://t3.tianditu.com/DataServer?T=cia_w&x={x}&y={y}&l={z}",
		terrlabel: "http://t3.tianditu.com/DataServer?T=cta_w&x={x}&y={y}&l={z}"
	},
	contentUtil.crs = {
		4326: "EPSG:4326",
		3857: "EPSG:3857"
...
layerUtil.createTiledLayer = function(a, b, c) {
	...
	layerUtil.createPointJsonLayer = function(a, b,d) {
		var c = commonUtil.pointToGeoJson(a, b.x, b.y,d);
...
mapUtil.createMap = function(a, b, c, d, e, f) {
	...
	mapUtil.addLayerPopup = function(map, layer, event, htmlFun) {
		var bool = false;
		var selectInteraction;
		var assoLayers = [];

		var bool_click = false;
		var selectInteraction_click;
		var assoLayers_click = [];
		for(var i = 0; i < map.getInteractions().getArray().length; i++) {
			var interaction = map.getInteractions().getArray()[i];
			if(interaction.get('selectId') == 'select') {
				bool = true;
				selectInteraction = interaction;
			}

			if(interaction.get('selectId') == 'select_click') {
				bool_click = true;
				selectInteraction_click = interaction;
			}
		}
		if(event == 'move') {
			if(bool) {
				assoLayers = selectInteraction.get('assoLayers');
				if(assoLayers.indexOf(layer) < 0) {
					assoLayers.push(layer);
					//关联callback事件
					layer.set('popupFun', (htmlFun ? htmlFun : undefined));
				}
			...
			selectInteraction.set('selectId', 'select');
			selectInteraction.set('assoLayers', assoLayers);
			...
			selectInteraction.on('select', function(e) {
				if(e.selected.length > 0) {
					...
					var layerCallback = layer.get('popupFun');
					var value = layerCallback ? layerCallback(values, feature, layer) : {};
					if(value.content == '') {
						return;
					}
					var content = value.content || "";
					var title = value.title || "";
					map.getOverlayById('popup').show(ol.extent.getCenter(feature.getGeometry().getExtent()), content, title);
				} else
					map.getOverlayById('popup').hide();
			});

		} else {
			if(bool_click) {
				assoLayers_click = selectInteraction_click.get('assoLayers');
				if(assoLayers_click.indexOf(layer) < 0) {
					assoLayers_click.push(layer);
					layer.set('popupFun_click', (htmlFun ? htmlFun : undefined));
				}
				map.removeInteraction(selectInteraction_click);
			} else {
				assoLayers_click.push(layer);
				layer.set('popupFun_click', (htmlFun ? htmlFun : undefined));
			}

			selectInteraction_click = new ol.interaction.Select({
				layers: assoLayers_click,
				style: styleSelect
			});
			selectInteraction_click.set('selectId', 'select_click');
			selectInteraction_click.set('assoLayers', assoLayers_click);
			map.addInteraction(selectInteraction_click);
			selectInteraction_click.on('select', function(e) {
				if(e.selected.length > 0) {
					var feature = e.selected[0];
					var values = feature.getProperties();
					var layer = selectInteraction_click.getLayer(feature);
					var layerCallback = layer.get('popupFun_click');
					var value = layerCallback ? layerCallback(values, feature, layer) : {};
					if(value.content == '') {
						return;
					}
...
mapUtil.addLayerEvent = function(map, layer, event, htmlFun) {
   ...
   switch (event) {
	   ...
	   case  'move':
           for(var i = 0; i < map.getInteractions().getArray().length; i++) {
               let interaction = map.getInteractions().getArray()[i];
               if(interaction.get('selectId') == 'moveSelect') {
                   selectInteraction = interaction;
               }
           }
           ...
           selectInteraction.set('selectId', 'moveSelect');
           selectInteraction.set('assoLayers', assoLayers);
           map.addInteraction(selectInteraction);
           selectInteraction.on('select', function(e) {
               if (e.selected.length > 0) {
	   ...
	   case 'click':
               for(var i = 0; i < map.getInteractions().getArray().length; i++) {
                   let interaction = map.getInteractions().getArray()[i];
                   if(interaction.get('selectId') == 'clickSelect') {
                       selectInteraction = interaction;
                   }
               }
               ...
               selectInteraction.set('selectId', 'clickSelect');
               selectInteraction.set('assoLayers', assoLayers);
               map.addInteraction(selectInteraction);
               selectInteraction.on('select', function(e) {
                   if (e.selected.length > 0) {
                       ...
                       if(layer.constructor ===ol.layer.AnimatedCluster){
                       ...
                       let layerCallback = layer.get('popupFun');
                       layerCallback ? layerCallback(feature, layer) : {};
                   }else{
				   }
               })
